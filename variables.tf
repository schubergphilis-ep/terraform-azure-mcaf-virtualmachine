...
- `<map key>` - Use a custom map key to define each data disk
  ...
  - `storage_account_type` (Required) - The type of storage to use for the managed disk. Possible values are Standard_LRS, StandardSSD_ZRS, Premium_LRS, PremiumV2_LRS, Premium_ZRS, StandardSSD_LRS or UltraSSD_LRS. Note: ZRS SKUs (`StandardSSD_ZRS`, `Premium_ZRS`) are zone-redundant and cannot be pinned to a single availability zone; for these SKUs `var.zone` is automatically ignored (set to `null`) to avoid the "SKU does not support availability zones" error.
...
- `<map key>` - Use a custom map key for the shutdown schedule definition
  ...
  - `tags` = (Optional) - Tags to apply to the shutdown schedules resource. Defaults to the module level `tags` when omitted.
